...
function olyCid() {
  try {
    return (localStorage.getItem('oly_google_cid') || '').trim();
  } catch (e) {
    return '';
  }
}
function olyLinked() {
  try {
    return localStorage.getItem('oly_google_linked') === '1';
  } catch (e) {
    return false;
  }
}
function setOlyLinked(v) {
  try {
    v
      ? localStorage.setItem('oly_google_linked', '1')
      : localStorage.removeItem('oly_google_linked');
  } catch (e) {}
}
function saveOlyToken(t, expSec) {
  try {
    localStorage.setItem(
      'oly_google_tok',
      JSON.stringify({ t, exp: Date.now() + (expSec || 3600) * 1000 }),
    );
  } catch (e) {}
}
...
function syncToken() {
  return readTokenRecord('day_google_tok') || readTokenRecord('oly_google_tok');
}

// ── GIS (standalone only — embedded rides Day's session) ──────────────────────
function olyGisAvailable() {
...
function initOlyTokenClient() {
  ...
  olyTokenClient = google.accounts.oauth2.initTokenClient({
    ...
    error_callback: () => {
      if (!olySilent) alert('Google sign-in cancelled');
      olySilent = false;
      setSyncStatus(olyLinked() ? 'tap Connect to re-sync' : 'not connected');
    },
  });
...
function connectOlyGoogle() {
  ...
  if (cidInput && cidInput.value.trim()) {
    try {
      localStorage.setItem('oly_google_cid', cidInput.value.trim());
    } catch (e) {}
    olyTokenClient = null;
  }
  if (syncToken()) {
  ...
  if (!olyCid()) {
    alert('Paste the Google Client ID first (same one as in Day’s Settings).');
    return;
  }
  if (!initOlyTokenClient()) {
    alert('Google library still loading — try again in a second.');
    return;
  }
...
function trySilentOlyConnect() {
  ...
  olySilent = true;
  try {
    olyTokenClient.requestAccessToken({ prompt: '' });
    return true;
  } catch (e) {
    olySilent = false;
    return false;
...
// ── Local state helpers ───────────────────────────────────────────────────────
function localOlyRaw() {
  try {
    const state = JSON.parse(localStorage.getItem('oly_state')) || null;
    const backup = JSON.parse(localStorage.getItem('oly_rev6_backup')) || null;
    return backup?.revision === 6 && (state?.revision !== 6 || backup.ts > state.ts)
      ? backup
      : state;
  } catch (e) {
    return null;
...
function stampLocalOly() {
  ...
  s.ts = Date.now();
  try {
    localStorage.setItem('oly_state', JSON.stringify(s));
  } catch (e) {}
}
...
async function driveApi(url, opts, tok) {
  ...
  if (r.status === 401) {
    try {
      localStorage.removeItem('oly_google_tok');
    } catch (e) {}
...
async function putRemote(tok) {
  ...
  let durations = null;
  try {
    durations = JSON.parse(localStorage.getItem('oly_day_durations'));
  } catch (e) {}
...
function adoptRemote(remote) {
  if (STATE.revision === 6 && remote.state?.revision !== 6) {
    setSyncStatus('Previous-program cloud copy ignored; current Revision 6 data retained.');
    return false;
  }
  try {
    MODEL.migrate(remote.state);
  } catch (e) {
    setSyncStatus('Cloud data was invalid; local data retained.');
    return false;
  }
  if (STATE.activeWorkout && remote.state.activeWorkout?.id !== STATE.activeWorkout.id) {
    setSyncStatus('Finish the active workout before pulling a different cloud session.');
    return false;
  }
  try {
    localStorage.setItem('oly_state', JSON.stringify(remote.state));
  } catch (e) {
    setSyncStatus('Could not save cloud data on this device.');
    return false;
  }
  try {
    localStorage.setItem('oly_rev6_backup', JSON.stringify(remote.state));
  } catch (e) {
    /* Primary copy was saved. */
  }
  if (remote.durations && remote.durations.min) {
    try {
      localStorage.setItem('oly_day_durations', JSON.stringify(remote.durations));
    } catch (e) {}
  }
...
async function syncNow() {
  ...
  if (!tok) {
    setSyncStatus(
      IS_EMBEDDED
        ? 'waiting for Day’s Google sign-in'
        : olyLinked()
          ? 'tap Connect to re-sync'
          : 'not connected',
    );
    return;
  }
  ...
  try {
    ...
    if (remote?.state && remote.state.revision !== 6) {
      // Preserve a newer/different old-program cloud copy without reverting the
      // new program. An empty fresh installation must not erase cloud history.
      MODEL.migrate(remote.state);
      const raw = JSON.stringify(remote.state);
      STATE.legacyArchives = STATE.legacyArchives || [];
      if (!STATE.legacy) STATE.legacy = remote.state;
      else if (
        JSON.stringify(STATE.legacy) !== raw &&
        !STATE.legacyArchives.some((s) => JSON.stringify(s) === raw)
      )
        STATE.legacyArchives.push(remote.state);
      if (!save()) throw new Error('Local archive could not be saved');
      await putRemote(tok);
      setSyncStatus('synced ✓ · previous-program cloud data preserved in History');
      return;
    }
    if (remote && remote.ts && (STATE.pristine || remote.ts > lts)) {
      if (!adoptRemote(remote)) return;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible' && syncToken() && Date.now() - _lastPullTs > 60000)
      syncNow();
  });
});
