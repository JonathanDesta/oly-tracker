...
The App 7.18 regional allocation lives in `src/dose.js`: explicit exercise targets, four restart stages, regional accounting and per-region decisions. New rows belong to the amended profile only. Legacy dose versions remain valid for frozen sessions, while upgrading resets stable-dose review and caps introduction at stage 3. Timers and Planner derive from the actual rows. Regional targets are programming decisions and do not represent guaranteed individual optima.
