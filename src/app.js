...
function failureGuide() {
  ...
  <p><strong>Dose and calendar:</strong> ordinary weekday order remains Monday B, Tuesday C, Thursday A, Friday D. The restart has four stages: an ordinary Foundation week has 34/42/62/80 conventional sets and 9/9/11/15 Olympic sets. Each complete green weekly review may advance one stage, with next-session follow-ups required. Good historical recovery supports trying the progression; it does not skip observation. Record the next-session checks in History. Then review two full green weeks at a stable workload before optional additions. A controlled addition can now add one weekly Olympic or conventional set to a selected existing exercise. Source heavy-slot and bounded-assessment rules remain superseded. Existing logs and active sessions keep their original prescription.</p>
...
function settingsView() {
  ...
  return (
    ...
    `<section class="panel"><h2>Weekly schedule</h2><p>${esc(scheduleName(t))}</p><p>Weekday plan: Mon B, Tue C, Thu A, Fri D. ${allLoadedFailure(t) ? "Week 12 uses Monday failure work and Friday fixed-load benchmarks, with moderate bench afterward. Tuesday/Thursday contain no loaded work." : "Week 12 retains the original taper/test order."} Switching an already-started week takes effect after its review.</p>${t.nextSchedule ? notice(`Pending next week: ${t.nextSchedule === "weekday" ? "weekday plan" : "source plan"}.`) : ""}<form data-form="schedule">${select("Training calendar", "schedule", { weekday: "Weekday plan · Mon B / Tue C / Thu A / Fri D", source: "Original PDF order · Mon A / Tue B / Thu C / Fri D" }, t.nextSchedule || t.schedule)}<p class="form-error" role="alert"></p>${submit("Save training calendar")}</form></section>` +
    ...
    `<section class="panel"><h2>Technique & interference</h2><form data-form="technique"><div class="input-grid">${["snatch", "clean", "jerk"].map((k) => select(pretty(k), k, allLoadedFailure(t) ? { none: "Ordinary failure prescription", receive: "Defer affected failure work for technique / receiving review", ...(t.technique[k] !== "none" && t.technique[k] !== "receive" ? { [t.technique[k]]: "Existing technique issue · affected work deferred" } : {}) } : k === "jerk" ? { none: "Ordinary prescription", stance: "Light split stance/recovery", dip: "Light pause-dip regression" } : { none: "Ordinary prescription", receive: "Unsafe receiving · technique-bar rehearsal", return: "Secure return · 4 singles at 40–60%", turnover: "High-hang turnover replacement", balance: "First two sets knee-pause" }, t.technique[k])).join("")}</div>${check(allLoadedFailure(t) ? "Omit A snatch failure set: repeated next-session cost" : "Reduce A snatch doubles to 4 × 2: repeated next-session cost", "reduceA", t.reduceA)}${check(allLoadedFailure(t) ? "Omit C jerk failure set while reviewing recovery" : "One fewer C jerk set for two exposures; suspend C assistance", "reduceJerk", t.reduceJerk)}${check("Lower-block fatigue: curls/calves 1; omit extensions/crunch", "lowerDose", t.lowerDose)}${check("Omit week-11 D affected lower work for slow recovery", "omitLastLower", t.omitLastLower)}${check("Omit provisional C pulls after target/cost review", "omitPull", t.omitPull)}${textarea("Observed issue / target and return review", "reason", "", "required")}<p class="form-error" role="alert"></p>${submit("Save technical prescription")}</form>${sourceLink(16)}${sourceLink(34)}</section>` +
